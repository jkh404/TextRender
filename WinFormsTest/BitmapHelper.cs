...
namespace WinFormsTest
{
    internal static class BitmapHelper
    {
        ...
        {
            using var readDataLock = frameRender.CreateReadDataLock();
            ...
            if (frameRender.Width==bitmap.Width)
            {
                ...
                readDataLock.Bytes.Slice(0, size).CopyTo(new Span<byte>(_ptr, size));
            }
            else if (frameRender.Width>bitmap.Width)
            {
                for (int y = 0; y < Height; y++)
                {
                    ...
                    var _temp= readDataLock.Bytes.Slice(y*frameRender.Width*4, Stride);
            ...
            else 
            {
                ...
                for (int y = 0; y < Height; y++)
                {
                    var offset = y * data.Stride;
                    var Stride = pixSize*Width;
                    readDataLock.Bytes.Slice(y*Stride, Stride).CopyTo(new Span<byte>(_ptr+offset, Stride));
                }
            }
